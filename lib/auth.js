...
async function storeCookies(session, response, url) {
  ...
  for (const h of headers) {
    ...
    await new Promise((resolve) => {
      session.cookieJar.setCookie(clean, url, { ignoreError: true }, () => resolve());
    });
  }
}
...
function generateFingerprint() {
  ...
  // djb2 hash matching the HTTP client script on class.tiflab.my.id
...
function normalizeUrl(url) {
  if (!url) return url;
  return url.replace(/^https:\/\/class\.tiflab\.my\.id/i, 'http://class.tiflab.my.id');
}

/**
 * Make an HTTP request with session cookies and realistic browser fingerprint headers
 */
async function sessionRequest(session, method, rawUrl, options = {}) {
  const url = normalizeUrl(rawUrl);
  const cookieString = await getCookieString(session, url);
  
  // Destructure headers out so the rest-spread doesn't overwrite our merged headers
...
async function login(username, password) {
  ...
  while ([301, 302, 303, 307, 308].includes(loginPage.status) && redirectCount < 5) {
    ...
    const fullUrl = normalizeUrl(redirectLocation.startsWith('http') ? redirectLocation : `${BASE_URL}${redirectLocation}`);
    console.log(`[auth] Following redirect (${loginPage.status}) to ${fullUrl}`);
    loginPage = await sessionRequest(session, 'GET', fullUrl);
  }

  // If HTTP returned a challenge or empty data, try HTTPS as fallback
  if (!loginPage.data || (typeof loginPage.data === 'string' && loginPage.data.includes('Just a moment...'))) {
    console.log('[auth] Trying HTTPS fallback for login page...');
    loginPage = await sessionRequest(session, 'GET', `https://class.tiflab.my.id/login`);
  }
...
async function parseLoginPage(session, loginPage, username, password) {
  ...
  if (loginResponse.status === 302 || loginResponse.status === 303 || loginResponse.status === 301) {
    ...
    const dashUrl = normalizeUrl(redirectLocation.startsWith('http') ? redirectLocation : `${BASE_URL}${redirectLocation}`);
