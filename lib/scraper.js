...
async function getCourses(session) {
  console.log('[scraper] Fetching course list from courses & dashboard...');

  const [coursesResp, dashResp] = await Promise.all([
    sessionRequest(session, 'GET', `${BASE_URL}/courses`).catch(() => null),
    sessionRequest(session, 'GET', `${BASE_URL}/dashboard`).catch(() => null),
  ]);

  const html1 = (coursesResp && coursesResp.status === 200 && typeof coursesResp.data === 'string') ? coursesResp.data : '';
  const html2 = (dashResp && dashResp.status === 200 && typeof dashResp.data === 'string') ? dashResp.data : '';
  ...
  const rawCourses = parseCourses(combinedHtml, '/courses');

  console.log('[scraper] Verifying course enrollments and titles...');
  const checkResults = await Promise.all(
    rawCourses.map(async (course) => {
      try {
        const chkResp = await sessionRequest(session, 'GET', course.url);
        if (chkResp.status !== 200) return null;
        const txt = typeof chkResp.data === 'string' ? chkResp.data : '';

        if (txt.includes('Enroll in Course') || (txt.includes('Enroll Now') && !txt.includes('Enrolled'))) return null;
        ...
        // Parse clean title directly from page heading if available
        const hMatch = txt.match(/<h[1-3][^>]*>([\s\S]*?)<\/h[1-3]>/i) || txt.match(/<title>([\s\S]*?)<\/title>/i);
        if (hMatch) {
          let name = hMatch[1].replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim();
          ...
          if (name && name.length >= 2 && !name.toLowerCase().includes('course') && !name.toLowerCase().includes('dashboard')) {
            course.name = name;
          }
        }
  ...
  return { courses: finalCourses, rawHtml: combinedHtml, pageUrl: `${BASE_URL}/courses` };
}
